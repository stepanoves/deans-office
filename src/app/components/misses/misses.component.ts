...
export class MissesComponent implements OnInit {
  ...
  public ngOnInit(): void {
    this.columns = [
      { key: 'department', header: 'Кафедра', cell: row => `${ row.department.name }`},
      ...
      { key: 'subject', header: 'Дисциплина', cell: row => `${ row.subject.name }`},
      { key: 'miss_type', header: 'Тип пропуска', cell: row => `${ row.miss_type }`},
      { key: 'date', header: 'Дата', cell: row => `${ row.date }`},
      { key: 'quantity_hours', header: 'Кол-во часов', cell: row => `${ row.quantity_hours }`},
      { key: 'hour_cost', header: 'Стоимость часа', cell: row => `${ row.hour_cost }`},
      { key: 'teacher', header: 'Преподаватель', cell: row => `${ row.teacher.name }`},
      { key: 'teacher_rank', header: 'Статус', cell: row => `${ row.teacher.rank }`},
      { key: 'paided', header: 'Оплачено', cell: row => `${ row.paided ? 'Да' : 'Нет' }`},
      { key: 'worked', header: 'Отработано', cell: row => `${ row.worked ? 'Да' : 'Нет' }`},
      { key: 'foreigner', header: 'Иностранец', cell: row => `${ row.foreigner ? 'Да' : 'Нет' }`},
    ];
  }
