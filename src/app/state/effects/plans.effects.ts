...
export class PlansEffects {
  ...
  addSubject$ = this.actions$.pipe(
    ...
    switchMap(({ planId, subject }) =>
      this.plansService
        .postPlanSubjects(planId, subject)
        .pipe(
          map(
            () => new plansActions.AddPlanSubjectSuccess(subject),
  ...
  updateSubject$ = this.actions$.pipe(
    ...
    switchMap(({ planId, subject }) =>
      this.plansService
        ...
        .pipe(
          map(
            () => new plansActions.UpdatePlanSubjectSuccess(subject),
            catchError(err => of(new plansActions.AddPlanSubjectFail(err))),
          ),
        ),
    ),
  ...
  constructor(private actions$: Actions, private plansService: PlansService) {
  }
}
